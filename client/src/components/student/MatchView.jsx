...
// Solo, so it's always your turn. Alongside the choices, the Mission Panel
// shows your console coming online (uplink → vitals → back room → full room)
// and the six phases passing — the build-status view that replaces a map
// (spec §1, §3).

import { useEffect, useState } from 'react';
import { emitAck, errorText } from '../../services/socket.js';
import { Art } from '../../services/assets.jsx';
import MissionPanel from '../shared/MissionPanel.jsx';
import MetersBar from '../shared/MetersBar.jsx';

const missionScore = (m) => (m ? (m.mission || 0) + (m.crewSafety || 0) + (m.team || 0) : 0);
...
export default function MatchView({ state, dispatch }) {
  ...
  const crewLow = (match.meters?.crewSafety ?? 50) <= 15;
  const otherLow = Object.entries(match.meters || {}).find(([k, v]) => k !== 'crewSafety' && v <= 15);

  return (
    <div className="match">
      <header className="match-header">
        <div className="nation-chip mission">🚀 <b>Mission Control: Texas in Space</b></div>
        <div className="trail-chip" title="Everyone runs the same console">Your Console</div>
        <div className="hold-chip" title="Your three meters added up (max 300)">
          Mission Score <b>{missionScore(match.meters)}</b><span className="muted"> / 300</span>
        </div>
        {phase && (
      ...
      <div className="meters-row solo">
        <MetersBar meters={match.meters} meta={meta} title="Your Console" />
      </div>

      {crewLow && !feedback && (
        <div className="banner danger" role="alert">
          ❤️ Crew Safety is running very low. Crew Safety always outranks the mission timeline. Work the problem.
        </div>
      )}
      {!crewLow && otherLow && !feedback && (
        <div className="banner danger" role="alert">
          ⚠️ Your {meta.meters[otherLow[0]]?.name || otherLow[0]} is running very low. Tend to it.
        </div>
      )}
      ...
      <div className="match-body">
        <section className="action-panel" aria-live="polite">
          {feedback ? (
            ...
            <div className="waiting-panel"><div className="pulse-dot" aria-hidden="true" /><p>Out on the range…</p></div>
          )}
        </section>

        <section className="map-panel">
          <MissionPanel meters={match.meters} chapterIndex={chapterIndex} />
        </section>
      </div>
    </div>
  );
...
const VERDICT_UI = {
  right: { label: 'The calm, checklist call', className: 'right', icon: '✓' },
