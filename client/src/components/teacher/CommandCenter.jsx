...
// the PDF → End Session (deletes everything from memory). Mission Control:
// Texas in Space is solo: everyone works the same console and starts as soon
// as they're approved — no pairing, and one class-wide accuracy group (spec §1, §6).
  ...
  if (phase === 'gate') {
    return (
      <div className="app teacher-app">
        ...
        <p className="muted">Mission Control: Texas in Space · everyone works the same console · TEKS 7.12C, 7.19C</p>
  ...
  return (
    <div className="app teacher-app">
      <header className="row">
        <div>
          <h1>Teacher Command Center</h1>
          <p className="muted">Mission Control: Texas in Space · session-only data · download the PDF before you end</p>
        </div>
      ...
      {pending.length > 0 && (
        <div className="card">
          ...
          <p className="muted">Approve a name and that student takes their console right away.</p>
          {pending.map((s) => (
      ...
      <div className="card">
        ...
        <div className="class-accuracy">
          <div className="trail-stat wide">
            <div className="trail-stat-name"><span aria-hidden="true">🚀</span> Your Console — the whole class</div>
      ...
      <footer className="app-footer">7th Grade Texas History · Mission Control: Texas in Space · TEKS 7.12C, 7.19C</footer>
    </div>
  );
}
...
function downloadPdf(roster) {
  ...
  const doc = new jsPDF();
  const BRAND = [26, 58, 122]; // mission blue

  doc.setFontSize(16);
  doc.text('Mission Control: Texas in Space — Session Report', 14, 16);
  doc.setFontSize(10);
  doc.setTextColor(90);
  ...
  autoTable(doc, {
    ...
    body: [['Your Console', String(cls.count), cls.count ? `${cls.average}%` : '—']],
    styles: { fontSize: 9 },
    headStyles: { fillColor: BRAND },
  });
  ...
  doc.text('7th Grade Texas History · Mission Control: Texas in Space · TEKS 7.12C, 7.19C', 14, doc.internal.pageSize.getHeight() - 8);

  doc.save(`mission_control_texas_in_space_${roster.joinCode}_${date}.pdf`);
}
