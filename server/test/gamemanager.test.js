...
// and inspects the emit instructions it returns. No sockets involved. Mission
// Control is solo with ONE class-wide role (a single side, no rival), so
// these focus on the solo lifecycle, one class-wide accuracy group, and —
// because this game has NO early-fail — that every run plays all 12 actions
// through to match:end.
import test from 'node:test';
import assert from 'node:assert/strict';
import { GameManager } from '../src/GameManager.js';
import game, { PHASES } from '../src/games/missionControl.js';
...
test('the default game is Mission Control: Texas in Space', () => {
  const manager = new GameManager();
  const joinCode = makeSession(manager);
  assert.equal(manager.registry.get(joinCode).gameId, 'missionControl');
});
...
test('solo student runs the mission and completes with 100% accuracy → Flight, We’re GO', () => {
  const manager = new GameManager();
  const joinCode = makeSession(manager);
  ...
  assert.equal(begin[0].payload.side, SIDE, 'the one side is the controller');
  assert.equal(begin[0].payload.mode, 'solo');
  assert.equal(begin[0].payload.chapterCount, 6, 'six phases');
  assert.equal(begin[0].payload.rivalMeters, null, 'no rival');
  assert.deepEqual(begin[0].payload.meters, { mission: 50, crewSafety: 50, team: 50 });
  ...
  assert.equal(end[0].payload.you.ending.key, 'top', 'a perfect run is Flight, We’re GO');
  assert.equal(end[0].payload.you.failed, false);
  assert.ok(end[0].payload.you.score >= 200);
...
test('all-wrong run plays all 12 actions to the end (no early-fail) → Long Night in the Trench, 0%', () => {
  const manager = new GameManager();
  const joinCode = makeSession(manager);
...
test('class accuracy is one class-wide group (the controller)', () => {
  const manager = new GameManager();
  const joinCode = makeSession(manager);
...
test('a wrong-kind move is rejected', () => {
  ...
  const joinCode = makeSession(manager);
  const res = join(manager, joinCode, 'Ana');
  // Phase 1 step 1 is a decision; submit a map action instead.
...
test('rejoin returns a full snapshot of the live turn', () => {
  ...
  const joinCode = makeSession(manager);
  const res = join(manager, joinCode, 'Ana');
  playRight(manager, joinCode, res.studentId); // Phase 1 decision 1 done; decision 2 pending
