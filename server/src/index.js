...
app.get('/healthz', (_req, res) =>
  res.json({ ok: true, service: 'mission-control-texas-in-space', liveSessions: manager.registry.sessions.size }));
...
server.listen(config.port, () => {
  console.log(`[mission-control] listening on :${config.port} — sessions live in memory only`);
});
